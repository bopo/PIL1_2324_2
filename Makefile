...
distclean: clean
	...
	find internal -type f -name "[0-9]*.py" | xargs rm -rf
	rm -rf .pytest_cache
	rm -rf .vagrant
	rm -rf scripts
	rm -rf deployments
...
test: ## 运行测试脚本
	DJANGO_SETTINGS_MODULE=config.settings.test poetry run python manage.py test tests --traceback -v1


start: ## 运行全部服务
	poetry run honcho start

shell: ## 运行 django shell_plus
	DJANGO_SETTINGS_MODULE=config.settings.dev poetry run python manage.py shell_plus

requirements:
	poetry export --without-hashes --without-urls --with=main -o requirements.txt
...
compose: requirements ## 编译 docker compose 镜像
	# pip install -U docker-squash

	docker build . -f compose/postgres/Dockerfile -t postgres:keeping
# 	docker build . -f compose/django/Dockerfile -t keeping:onbuild --build-arg VERSION="$(VERSION)"
	docker build . -f compose/django/Dockerfile -t keeping:$(VERSION)

# 	docker-squash keeping:onbuild -t keeping:squash

# 	docker rmi keeping:squash
	docker image list

#compose:
#	docker compose run --volume `pwd`/service:/app/service --rm server python3 manage.py migrate
#	docker compose run --volume `pwd`:/app --rm server python3 manage.py keepingtatic --no-input
...
upgrade: migrate ## 升级项目, 使用变量 MODE 作为参数 (例如: `make upgrade MODE=pro`)
	DJANGO_SETTINGS_MODULE=config.settings.$(MODE) poetry run python manage.py loaddata fixtures/seeds/*.json
...
fixture:
	poetry run python manage.py loaddata fixtures/user.json
	poetry run python manage.py keepingtatic --noinput
...
# 	docker save keeping:$(VERSION) -o dist/keeping-$(VERSION).tar.gz
#
# 	ls -lht dist
...
# 	cp -R fixtures/*.json build/fixtures
#
...
#
# 	docker save keeping:$(VERSION) -o build/docker/keeping-$(VERSION).tar.gz
# 	shasum ./build/docker/keeping-$(VERSION).tar.gz >./build/docker/keeping-$(VERSION).tar.gz.md5
#
# 	docker save postgres:keeping -o build/docker/postgres-$(VERSION).tar.gz
# 	shasum ./build/docker/postgres-$(VERSION).tar.gz >./build/docker/postgres-$(VERSION).tar.gz.md5
#
# 	tar zcfv ./dist/keeping-$(VERSION).tar.gz build/
# 	ls -lht ./dist
#
